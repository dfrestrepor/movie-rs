...
# Usage Example: python3 scripts/create_cfn_stack.py  --environment dev --ec2-key-name movie-key-ec2
...
def main():
    ...
    bucket_name = f'movie-sr-bootstrap-{account_id}-{region}'
    ...
    # create cfn stack
    stack_name = f'movie-sr-{args.environment}'
    cfn_template_path = f'{dir_path}/cloudformation/movie-rs.yml'
    cfn_params_path = f'{dir_path}/cloudformation/movie-rs-params-{args.environment}.json'
...
def put_ssm_parameter(bucket_name):
    try:
        response = ssm_client.put_parameter(
            Name='/movie_sr/bootstrap_bucket',
